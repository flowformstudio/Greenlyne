...
function PipelineDesktop() {
  ...
  const [freshlyAddedId, setFreshlyAddedId] = useState(() => {
    // Hand-off from Geo Campaigns — when the user pushes a prescreened lead
    // into the CRM via "+CRM", the Geo page sets this in sessionStorage and
    // navigates here. We pick it up once and highlight that lead.
    try {
      const id = sessionStorage.getItem('pipeline_freshly_added')
      if (id) {
        sessionStorage.removeItem('pipeline_freshly_added')
        sessionStorage.removeItem('pipeline_freshly_added_at')
        setTimeout(() => setFreshlyAddedId(null), 12000)
        return id
      }
    } catch {}
    return null
  ...
  const fundedVal          = LEADS.filter(l => l.status === 'funded').reduce((s, l) => s + (parseInt(String(l.amount ?? '').replace(/[$,]/g, '')) || 0), 0)
              ...
              <div className="absolute right-0 top-full mt-1.5 z-40 rounded-xl overflow-hidden"
                ...
                {[
                  ...
                  },
                  {
                    ...
                    action: () => { navigate('/geo-campaigns?view=map&from=pipeline'); setAddLeadsOpen(false) },
                  },
              ...
              <div className="rounded-xl p-4 flex flex-col gap-2" style={{background: tk.fundedCardBg, border:`1px solid ${tk.fundedCardBorder}`}}>
                ...
                <div className="mt-auto pt-3" style={{borderTop:`1px solid ${tk.fundedBorderTop}`}}>
                  <span className="text-[10px]" style={{color: tk.fundedMutedColor}}>
                    Pipeline value: ${Math.round(LEADS.filter(l => !['funded'].includes(l.status)).reduce((s,l)=>s+(parseInt(String(l.amount ?? '').replace(/[$,]/g,'')) || 0),0)/1000)}k
                  </span>
                </div>
              </div>
            </div>
              ...
              <tbody>
                ...
                {paginated.map((lead, i) => {
                  ...
                  <tr
                    ...
                    ref={el => { if (isFresh && el && lead.id === freshlyAddedId) el.scrollIntoView?.({ behavior: 'smooth', block: 'center' }) }}
