...
// ─── Customer & roof data ─────────────────────────────────────────────────────
// Roof capacity is fixed; identity comes from the demo session (set by PMPro's
// Quick Prescreen) so the calculator greets the same person the merchant
// prescreened. Read live inside the component via useCustomerIdentity().
const PANELS_MAX = 19

function useCustomerIdentity() {
  const s = getDemoSession()
  const first  = s.firstName || DEMO_PERSONA.firstName
  const street = s.address   || DEMO_PERSONA.address
  const city   = s.city      || DEMO_PERSONA.city
  const state  = s.state     || DEMO_PERSONA.state
  return {
    name:    first,
    address: `${street}, ${city} ${state}`.trim().replace(/,\s*$/, ''),
  }
}
...
function calcSavings(bill, activePanels) {
  const coverage = activePanels / PANELS_MAX
...
// ─── RoofScene SVG ───────────────────────────────────────────────────────────
function RoofScene({ activePanels, sunProgress, onTogglePanel, address }) {
  const W = 720, H = 560
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: 'block' }} preserveAspectRatio="xMidYMid slice">
      ...
      <g>
        ...
        <text x="46" y="68" fontSize="13" fill="#F5F1EE" fontFamily={DISPLAY} fontWeight="700" letterSpacing="-0.01em">{address}</text>
      </g>
    </svg>
  )
}
...
export default function OfferLanding() {
  ...
  const panelPct = Math.round(panels / PANELS_MAX * 100)
  ...
  const togglePanel = i => {
    ...
    setPanels(isActive ? Math.max(0, panels - 1) : Math.min(PANELS_MAX, panels + 1))
  }
      ...
      <main style={{
        ...
        <section>
          ...
          <div style={{ borderRadius: 20, overflow: 'hidden', background: 'linear-gradient(135deg, #07153a 0%, #122a5c 100%)', border: `1px solid ${T.ink10}`, boxShadow: '0 30px 80px -30px #00166077', position: 'relative' }}>
            <RoofScene activePanels={panels} sunProgress={sunProgress} onTogglePanel={togglePanel} address={customer.address} />
            ...
            <div style={{ position: 'absolute', bottom: 18, right: 18, padding: '10px 14px', borderRadius: 12, background: '#00000066', border: '1px solid #ffffff20', color: '#F5F1EE', display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <span style={{ fontFamily: DISPLAY, fontSize: 18, fontWeight: 700 }}>{panels} <span style={{ opacity: 0.5, fontSize: 13 }}>/ {PANELS_MAX}</span></span>
            </div>
          </div>
          ...
          <div style={{ background: T.white, border: `1px solid ${T.ink10}`, borderRadius: 16, padding: '18px 20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 12 }}>
              ...
              <div style={{ fontFamily: DISPLAY, fontSize: 22, fontWeight: 700, color: T.navy, letterSpacing: '-0.015em' }}>
                {panels}<span style={{ fontSize: 13, color: T.ink40, fontWeight: 500 }}> / {PANELS_MAX}</span>
              </div>
            </div>
            <input
              type="range" min="0" max={PANELS_MAX} step="1" value={panels}
