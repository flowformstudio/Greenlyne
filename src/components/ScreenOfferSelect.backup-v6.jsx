  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '18px 20px 22px', display: 'flex', flexDirection: 'column', flex: 1 }}>
        ...
        <div style={{ fontSize: 14, color: T.muted, lineHeight: 1.5, marginBottom: 0, minHeight: 42 }}>
          {isRecommended
            ? 'Reduced payments for 5 years'
            : 'One flat payment from day one'}
        </div>

        {/* Label, then amount + APR sharing the same baseline */}
        <div>
          <div style={{ fontSize: 12, fontWeight: 600, color: T.muted, letterSpacing: '0.005em', marginBottom: 4 }}>
            Total loan amount
          </div>
          <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 12 }}>
            <div style={{ fontSize: 33, fontWeight: 800, color: T.text, ...NUM, letterSpacing: '-0.03em', lineHeight: 1 }}>
              {formatCurrencyFull(Math.round(offer.loanAmount))}
            </div>
            <div style={{ fontSize: 12, fontWeight: 500, color: T.muted, ...NUM, whiteSpace: 'nowrap' }}>
              APR <span style={{ fontWeight: 600, color: T.body }}>{(offer.apr * 100).toFixed(2)}%</span>
            </div>
          </div>
        </div>

        {/* Savings slot — BELOW the amount; fixed reserved height so all tiles align */}
        <div style={{ minHeight: 26, marginTop: 8, marginBottom: 16, display: 'flex', alignItems: 'center' }}>
          {showSavings && (
            <span style={{
              display: 'inline-flex', alignItems: 'center', gap: 5,
              ...
              Pay {formatCurrencyFull(Math.round(myFive))} in first 5 years
            </span>
          )}
        </div>
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '18px 20px 22px', display: 'flex', flexDirection: 'column', flex: 1 }}>
        ...
        {/* Title + description — outside inner box */}
        <div style={{
          fontSize: 19, fontWeight: 700, color: T.white,
          letterSpacing: '-0.02em', marginBottom: 4,
          whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
        }}>
          Custom
        </div>
        <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.65)', lineHeight: 1.5, marginBottom: 0, minHeight: 42 }}>
          A custom plan, configured by you.
        </div>

        {/* Label, then amount + APR sharing the same baseline */}
        <div>
          <div style={{ fontSize: 12, fontWeight: 600, color: 'rgba(255,255,255,0.6)', letterSpacing: '0.005em', marginBottom: 4 }}>
            Total loan amount
          </div>
          <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 12 }}>
            <div style={{ fontSize: 33, fontWeight: 800, color: T.white, ...NUM, letterSpacing: '-0.03em', lineHeight: 1 }}>
              {formatCurrencyFull(Math.round(offer.loanAmount))}
            </div>
            <div style={{ fontSize: 12, fontWeight: 500, color: 'rgba(255,255,255,0.6)', ...NUM, whiteSpace: 'nowrap' }}>
              APR <span style={{ fontWeight: 600, color: T.white }}>{(offer.apr * 100).toFixed(2)}%</span>
            </div>
          </div>
        </div>

        {/* Reserved spacer (no savings on Custom) — keeps hero aligned with Recommended */}
        <div style={{ minHeight: 26, marginTop: 8, marginBottom: 16 }} />
