      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 18, fontWeight: 700, color: T.text, letterSpacing: '-0.02em', lineHeight: 1.2 }}>
              {calc ? (
                <>
                  <span style={{ ...NUM }}>{formatCurrencyFull(Math.round(calc.L))}</span> loan · <span style={{ ...NUM }}>{rateDisplay}</span> fixed rate
                </>
              ) : 'Configure your options above'}
            </div>
          </div>
        </div>
        <div>
          <button
            ...
            style={{
              ...
              fontFamily: 'inherit',
              boxShadow: calc ? '0 6px 20px rgba(37,75,206,0.35)' : 'none',
              transition: 'transform 0.15s, box-shadow 0.15s, background 0.15s',
              whiteSpace: 'nowrap',
            }}
            onMouseOver={e => { if (calc) { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 10px 28px rgba(37,75,206,0.4)'; e.currentTarget.style.background = T.blueHi } }}
            onMouseOut={e => { e.currentTarget.style.transform = ''; e.currentTarget.style.boxShadow = calc ? '0 6px 20px rgba(37,75,206,0.35)' : 'none'; if (calc) e.currentTarget.style.background = T.blue }}
          >
            Confirm your plan →
          </button>
        </div>
